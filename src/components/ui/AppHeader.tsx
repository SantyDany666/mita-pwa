...
export function AppHeader({
  ...
  return (
    <header
      className={cn(
        "bg-white dark:bg-gray-900 sticky top-0 z-20 shadow-sm border-b border-slate-200 dark:border-gray-800 pt-[env(safe-area-inset-top)]",
        className,
      )}
    >
      <div className="relative flex items-center justify-between px-4 h-14">
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
          <h1
            className={cn(
              "text-slate-900 dark:text-white text-lg font-bold leading-tight tracking-[-0.015em] text-center",
              titleClassName,
            )}
          >
            {title}
