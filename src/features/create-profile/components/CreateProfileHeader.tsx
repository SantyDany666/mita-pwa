  ...
  return (
    <header className="mb-8">
      <div className="flex items-center justify-between mb-6 h-10">
        {isFirstStep ? (
          <>
            <span className="text-xs font-semibold uppercase tracking-widest text-[#3E7CB1]">
            ...
            <div className="flex gap-1">
              {Array.from({ length: totalSteps }).map((_, i) => (
                <div
                  key={i}
                  className={`h-1.5 rounded-full ${i + 1 === step ? "w-10 bg-[#054A91]" : "w-6 bg-[#81A4CD]"
                    }`}
                />
              ))}
            </div>
          ...
          <>
            ...
            <div className="flex flex-col items-end">
              <span className="text-xs font-semibold text-[#3E7CB1] uppercase tracking-widest mb-1">
                Paso {step} de {totalSteps}
              </span>
              <div className="flex gap-1">
                {Array.from({ length: totalSteps }).map((_, i) => (
